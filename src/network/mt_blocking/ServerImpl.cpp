...
void ServerImpl::Stop() {
	std::lock_guard<std::mutex> _lock(_clients_mutex);
	running.store(false);
	_max_number = 0;
...
void ServerImpl::OnRun() {
	...
	while (running.load()) {
		...
		if (_logger->should_log(spdlog::level::debug)) 
		{
			...
			if (getnameinfo(&client_addr, client_addr_len, hbuf, sizeof(hbuf), sbuf, sizeof(sbuf), NI_NUMERICHOST | NI_NUMERICSERV) == 0) 
			{
				host = hbuf;
				port = sbuf;
			}
